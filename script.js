$(document).ready(function () {
  $("select#algo").change(function () {
    var idOld = $(this).children("option:selected").val();
    $("#data_" + idOld).show();

    $("select#algo").change(function () {
      $("#data_" + idOld).hide();
      var idNew = $(this).children("option:selected").val();
      $("#data_" + idNew).show();
    });
  });
});
...
document.getElementById("priority-toggle-btn").onclick = () => {
  let currentPriorityPreference = document.getElementById(
    "priority-preference"
  ).innerText;
  ...
  inputs.forEach((input) => {
    if (input.type == "number") {
      input.onchange = () => {
        ...
        } //min 1 : time quantum, priority, process time
        else {
...
function lcmAll() {
  ...
  for (let i = 0; i < process; i++) {
    result = lcm(
      result,
      document.querySelector(".main-table").rows[2 * i + 2].cells.length
    );
  }
  return result;
}
...
function showGanttChart(output, outputDiv) {
  ...
  function drawGanttChart() {
    ...
    let ganttWidth = "100%";
    if (startGantt >= 50) {
      ganttWidth = 0.001 * startGantt * screen.availWidth;
    }
    var options = {
      width: ganttWidth,
      timeline: {
...
function showTimelineChart(output, outputDiv) {
  ...
  function drawTimelineChart() {
    ...
    if (startTimeline >= 50) {
      timelineWidth = 0.001 * startTimeline * screen.availWidth;
    }
    var options = {
      width: timelineWidth,
    };
...
function showFinalTable(input, output, outputDiv) {
  ...
  let lastct = 0;
  output.completionTime.forEach(
    (element) => (lastct = Math.max(lastct, element))
  );
  ...
  if (input.contextSwitch > 0) {
    ...
    cs.innerHTML =
      "Number of Context Switches : " + (output.contextSwitches - 1);
    ...
    if (currentTimeLog.running.length == 1) {
      ...
      if (input.algorithm == "rr") {
        ...
        utility.remainingTimeRunning[found] = Math.min(
          utility.remainingProcessTime[found][
        ...
        candidates.sort((a, b) => {
          switch (input.algorithm) {
            ...
            case "pp":
              return (
                priorityPreference * (input.priority[a] - input.priority[b])
              );
          }
        });
        found = candidates[0];
      ...
      if (utility.start[found] == false) {
        ...
        output.responseTime[found] =
          currentTimeLog.time - input.arrivalTime[found];
      }
    }
    currentTimeLog.time++;
    if (found != -1) {
      ...
      if (input.algorithm == "rr") {
        ...
        if (
          ...
          utility.currentProcessIndex[found]++;
          if (
            utility.currentProcessIndex[found] == input.processTimeLength[found]
          ) {
            utility.done[found] = true;
            output.completionTime[found] = currentTimeLog.time;
            moveElement(
          ...
          if (currentTimeLog.running.length == 0) {
            ...
            for (
              let i = 0;
              i < input.contextSwitch;
              i++, currentTimeLog.time++
            ) {
              updateReadyQueue(currentTimeLog);
            }
            if (input.contextSwitch > 0) {
